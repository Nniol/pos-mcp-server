...
PRODUCTS: dict[str, dict] = {
    "P001": {"id": "P001", "name": "Espresso Blend Coffee",     "category": "Beverages",  "price": 12.99, "stock": 142},
    "P002": {"id": "P002", "name": "Oat Milk (1L)",              "category": "Dairy Alt",  "price":  2.49, "stock":  38},
    "P003": {"id": "P003", "name": "Croissant",                  "category": "Bakery",     "price":  3.50, "stock":  24},
    "P004": {"id": "P004", "name": "Green Tea Matcha",           "category": "Beverages",  "price":  9.99, "stock":  76},
    "P005": {"id": "P005", "name": "Sparkling Water (500ml)",    "category": "Beverages",  "price":  1.20, "stock": 200},
    "P006": {"id": "P006", "name": "Blueberry Muffin",           "category": "Bakery",     "price":  3.99, "stock":  18},
    "P007": {"id": "P007", "name": "Whole Milk (1L)",            "category": "Dairy",      "price":  1.89, "stock":  55},
    "P008": {"id": "P008", "name": "Dark Chocolate Bar",         "category": "Snacks",     "price":  4.50, "stock":  90},
    "P009": {"id": "P009", "name": "Granola Bar",                "category": "Snacks",     "price":  2.20, "stock":  11},
    "P010": {"id": "P010", "name": "Orange Juice (1L)",          "category": "Beverages",  "price":  3.75, "stock":   0},
}

TRANSACTIONS: list[dict] = [
    {"tx_id": "TX1001", "date": "2025-06-01", "product_id": "P001", "qty": 3, "total": 38.97, "terminal": "T1", "cashier": "Anna"},
    {"tx_id": "TX1002", "date": "2025-06-01", "product_id": "P003", "qty": 5, "total": 17.50, "terminal": "T2", "cashier": "Ben"},
    {"tx_id": "TX1003", "date": "2025-06-02", "product_id": "P004", "qty": 2, "total": 19.98, "terminal": "T1", "cashier": "Anna"},
    {"tx_id": "TX1004", "date": "2025-06-02", "product_id": "P008", "qty": 4, "total": 18.00, "terminal": "T3", "cashier": "Clara"},
    {"tx_id": "TX1005", "date": "2025-06-03", "product_id": "P001", "qty": 6, "total": 77.94, "terminal": "T2", "cashier": "Ben"},
    {"tx_id": "TX1006", "date": "2025-06-03", "product_id": "P006", "qty": 3, "total": 11.97, "terminal": "T1", "cashier": "Anna"},
    {"tx_id": "TX1007", "date": "2025-06-04", "product_id": "P002", "qty": 8, "total": 19.92, "terminal": "T3", "cashier": "Clara"},
    {"tx_id": "TX1008", "date": "2025-06-04", "product_id": "P005", "qty":10, "total": 12.00, "terminal": "T2", "cashier": "Ben"},
    {"tx_id": "TX1009", "date": "2025-06-05", "product_id": "P009", "qty": 2, "total":  4.40, "terminal": "T1", "cashier": "Anna"},
    {"tx_id": "TX1010", "date": "2025-06-05", "product_id": "P003", "qty": 7, "total": 24.50, "terminal": "T3", "cashier": "Clara"},
    {"tx_id": "TX1011", "date": "2025-06-05", "product_id": "P004", "qty": 1, "total":  9.99, "terminal": "T2", "cashier": "Ben"},
    {"tx_id": "TX1012", "date": "2025-06-06", "product_id": "P001", "qty": 4, "total": 51.96, "terminal": "T1", "cashier": "Anna"},
]

TERMINALS: dict[str, dict] = {
    "T1": {"id": "T1", "location": "Front Counter",  "cashier": "Anna",  "status": "online"},
    "T2": {"id": "T2", "location": "Drive-Through",  "cashier": "Ben",   "status": "online"},
    "T3": {"id": "T3", "location": "Self-Service",   "cashier": "Clara", "status": "offline"},
}
...
class ProductLookupInput(BaseModel):
    ...
    product_id: str = Field(..., description="Product ID (e.g. 'P001'). Use pos_list_products to find valid IDs.", min_length=1, max_length=10)

class CategoryFilterInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    category: Optional[str] = Field(default=None, description="Filter by category name, e.g. 'Beverages', 'Bakery', 'Snacks'. Omit to return all categories.")

class SalesReportInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    date_from: Optional[str] = Field(default=None, description="Start date filter in YYYY-MM-DD format, e.g. '2025-06-01'. Omit to include all dates.")
    date_to:   Optional[str] = Field(default=None, description="End date filter in YYYY-MM-DD format, e.g. '2025-06-05'. Omit to include all dates.")
    cashier:   Optional[str] = Field(default=None, description="Filter by cashier name, e.g. 'Anna'. Omit to include all cashiers.")

class InventoryAlertInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    threshold: int = Field(default=20, description="Stock quantity threshold. Products at or below this level are flagged. Default is 20.", ge=0, le=500)
...
async def pos_list_products(params: CategoryFilterInput) -> str:
    ...
    if params.category:
        ...
        if not products:
            ...
            return json.dumps({"error": f"No products in category '{params.category}'.", "available_categories": cats})
...
async def pos_sales_report(params: SalesReportInput) -> str:
    ...
    for t in txs:
        ...
        if pid not in by_product:
            ...
            by_product[pid] = {"product_id": pid, "name": name, "qty_sold": 0, "revenue": 0.0}
